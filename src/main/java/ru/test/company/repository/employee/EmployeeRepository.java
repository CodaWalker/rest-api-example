...
//    Long getCountAtWorkEmployeeInDepartment(Boolean f, UUID departmentId);
    ...
    @Query("SELECT count(e) FROM Employee e LEFT JOIN Calendar c ON e.id = c.employee.id WHERE  e.presenceAtWork = false AND e.department.id = :dIp AND c.event = :event AND c.startIntervalDate <= :start AND c.endIntervalDate >= :end")
