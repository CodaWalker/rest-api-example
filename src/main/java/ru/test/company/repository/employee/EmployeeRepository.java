...
//    Long getCountAtWorkEmployeeInDepartment(Boolean f, UUID departmentId);
    ...
    @Query("SELECT count(e) FROM Employee e LEFT JOIN Calendar c ON e.id = c.employee.id WHERE  e.presenceAtWork = false AND e.department.id = :dIp AND c.event = 'ABSENTED_HOLIDAY' AND c.startIntervalDate <= :start AND c.endIntervalDate >= :end")
    Long countEmployeesByDepartmentIdAndNew(@Param("dIp")UUID departmentId, @Param("start")LocalDate start, @Param("end")LocalDate end);

    @Query("SELECT count(e) FROM  Employee e LEFT JOIN Calendar c ON c.employee.id = e.id WHERE e.presenceAtWork = true AND e.department.id = :dIp")
